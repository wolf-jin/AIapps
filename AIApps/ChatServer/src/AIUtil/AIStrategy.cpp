...
// 🌟 架构师升级：接收 tuple，告别 % 2
json AliyunStrategy::buildRequest(const std::vector<std::tuple<int, std::string, long long>>& messages) const {
    json payload;
    payload["model"] = getModel();
    json msgArray = json::array();
...
// 🌟 架构师升级：接收 tuple，告别 % 2
json DouBaoStrategy::buildRequest(const std::vector<std::tuple<int, std::string, long long>>& messages) const {
    json payload;
    payload["model"] = getModel();
    json msgArray = json::array();
...
json AliyunRAGStrategy::buildRequest(const std::vector<std::tuple<int, std::string, long long>>& messages) const {
    json payload;
    json msgArray = json::array();
    for (const auto& msgItem : messages) {
        json msg;
...
json AliyunMcpStrategy::buildRequest(const std::vector<std::tuple<int, std::string, long long>>& messages) const {
    json payload;
    payload["model"] = getModel();
    json msgArray = json::array();
